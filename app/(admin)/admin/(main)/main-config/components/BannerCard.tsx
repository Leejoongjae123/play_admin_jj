...
export default function BannerCard({
  ...
  return (
    <div className={cn('flex w-full flex-col gap-4 rounded bg-[#FAF8F6] p-8', className)}>
      {/* Header */}
      <div className="flex items-center justify-between">
        <h3 className="font-pretendard text-xl font-bold text-gray-1">{title}</h3>
        <button type="button" onClick={onDelete} className="flex items-center gap-2">
      ...
      <div className="flex flex-col gap-1">
        ...
        <div className="flex items-center gap-[10px]">
          <div ref={startDatePickerRef} className="relative flex-1">
            <div onClick={handleStartDateClick}>
              <DatePicker value={startDate} className="flex-1 cursor-pointer" />
            </div>
            {showStartDatePicker && (
          ...
          <div ref={endDatePickerRef} className="relative flex-1">
            <div onClick={handleEndDateClick}>
              <DatePicker value={endDate} className="flex-1 cursor-pointer" />
            </div>
            {showEndDatePicker && (
              <div className="absolute left-0 top-full z-50 mt-2">
                <DateEdit
