  ...
  return (
    <DropdownMenu open={open} onOpenChange={setOpen} modal={false}>
      <DropdownMenuTrigger asChild>
        ...
        >
          <span className="font-pretendard text-xs font-bold text-[#911A00]">{value}</span>
          <UpArrow size={10} className={`transition-transform ${open ? 'rotate-180' : ''}`} />
        </button>
      </DropdownMenuTrigger>
      <DropdownMenuContent
        align="start"
        ...
        {options.map((option) => (
          <DropdownMenuItem
            ...
            className={`cursor-pointer px-4 py-2 hover:bg-[#FFF5F2] focus:bg-[#FFF5F2] ${
              value === option ? 'bg-[#FFF5F2] font-bold text-[#911A00]' : 'text-[#686868]'
            }`}
          >
