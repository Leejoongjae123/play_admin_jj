...
export default function PopupEditPage() {
  ...
  const [images, setImages] = useState<PopupImage[]>([
    { id: 1, image: '/sample-image.jpg', url: 'https://www.instagram.com/inscriptbooks/', isPublished: true },
    { id: 2, url: 'http://', isPublished: true },
    { id: 3, url: 'http://', isPublished: false },
    { id: 4, image: '/sample-image.jpg', url: 'https://www.instagram.com/inscriptbooks/', isPublished: false },
    { id: 5, url: 'http://', isPublished: true },
  ]);

  const handleImageToggle = (id: number) => {
    setImages(images.map(img => 
      img.id === id ? { ...img, isPublished: !img.isPublished } : img
    ));
  };
  ...
  const handleImageUpload = (id: number, file: File) => {
    setImages(images.map(img => 
      img.id === id ? { ...img, image: file } : img
    ));
  };

  const renderTimeSelector = (value: string, onChange: (value: string) => void, options: string[]) => (
    <div className="flex items-center gap-3 rounded-md border border-[#EBEBEB] bg-white px-3 py-3">
      <span className="font-pretendard text-xs font-normal text-gray-3">{value}</span>
      <ChevronDown size={12} color="#6D6D6D" />
    </div>
  );

  const renderImageCard = (image: PopupImage) => (
    <div key={image.id} className="flex w-[332px] flex-col gap-4 rounded-md bg-[#FAF8F6] p-8">
      <div className="flex items-center justify-between">
        ...
        >
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
            <path d="M7 21C6.45 21 5.97933 20.8043 5.588 20.413C5.19667 20.0217 5.00067 19.5507 5 19V6H4V4H9V3H15V4H20V6H19V19C19 19.55 18.8043 20.021 18.413 20.413C18.0217 20.805 17.5507 21.0007 17 21H7ZM17 6H7V19H17V6ZM9 17H11V8H9V17ZM13 17H15V8H13V17Z" fill="#D65856"/>
          </svg>
        ...
        <div className="relative">
          {image.image ? (
            <div 
              ...
              style={{ 
                backgroundImage: typeof image.image === 'string' 
                  ? `url(${image.image})` 
            ...
            <div className="flex h-[200px] flex-col items-center justify-center rounded-md border border-dashed border-gray-4 bg-[#F2F2F2]">
              <div className="flex flex-col items-center gap-2">
                <svg width="60" height="60" viewBox="0 0 60 60" fill="none">
                  <path d="M12.5008 38.5L21.4654 28.5354C22.403 27.598 23.6746 27.0714 25.0004 27.0714C26.3262 27.0714 27.5977 27.598 28.5354 28.5354L40.0004 40.0004M35.0004 35.0004L38.9654 31.0354C39.903 30.098 41.1746 29.5714 42.5004 29.5714C43.8262 29.5714 45.0977 30.098 46.0354 31.0354L48.001 34.5M35.0004 20.0004H35.0254M48.5359 48.5359V11.4648H11.4648V48.5359H48.5359Z" stroke="#A0A0A0" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"/>
                </svg>
                <div className="flex flex-col items-center gap-2">
                  <span className="font-pretendard text-lg font-bold text-gray-4">업로드하기</span>
                  <span className="font-pretendard text-xs font-normal text-gray-4 text-center">
                    가로 (width)는 최소 320px<br />이상을 권장합니다.
                  </span>
                </div>
              </div>
            </div>
      ...
      <Toggle 
        ...
        label={image.isPublished ? '게���' : '미게시'}
      />
    </div>
  );

  return (
    <div className="flex w-full max-w-[1180px] gap-[10px] p-8">
      <div className="flex flex-1 flex-col gap-20 rounded-md bg-white p-11">
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2">
                ...
                {renderTimeSelector(startHour, setStartHour, [])}
                {renderTimeSelector(startMinute, setStartMinute, [])}
              </div>
              <span className="font-pretendard text-xs font-bold text-gray-3">-</span>
              <div className="flex items-center gap-2">
                <DatePicker value={endDate} onChange={setEndDate} placeholder="날짜 입력" className="w-[140px]" />
                {renderTimeSelector(endHour, setEndHour, [])}
                {renderTimeSelector(endMinute, setEndMinute, [])}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex items-center gap-4">
            <button className="flex items-center gap-2 rounded-md bg-[#DEC6C1] px-[18px] py-[10px]">
              ...
              <span className="font-pretendard text-base font-normal text-primary">팝업 이미지 추가</span>
            </button>
          ...
          <div className="flex flex-wrap gap-4">
            {images.slice(0, 3).map(renderImageCard)}
          </div>
          <div className="flex gap-4">
            {images.slice(3, 5).map(renderImageCard)}
          </div>
        </div>
        ...
        <div className="flex w-[590px] items-center">
          ...
          <div className="flex items-center">
            <RadioGroup value={closeMethod} onValueChange={setCloseMethod} className="flex items-center gap-0">
              <div className="flex items-center gap-2 p-2">
                <RadioGroupItem value="today" className="h-6 w-6" />
                <span className={`font-pretendard text-base font-bold ${closeMethod === 'today' ? 'text-primary' : 'text-gray-3'}`}>
                  오늘 하루 열지 않음
                </span>
              </div>
              <div className="flex items-center gap-2 p-2">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <span className="font-pretendard text-base font-normal text-gray-3">일간 열지 않음</span>
                </div>
              </div>
              <div className="flex items-center gap-2 p-2">
                <RadioGroupItem value="never" className="h-6 w-6" />
                <span className="font-pretendard text-base font-normal text-gray-3">다시 열지 않음</span>
              </div>
            </RadioGroup>
          </div>
        </div>
